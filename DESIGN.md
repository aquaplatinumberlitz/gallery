...
- **Related Assets (`RelatedAssetsPanel.vue`, `RelationReasonList.vue`):** Image-level overflow action. Fixed tier label ("Same Recipe", "Visual Variants") with short reason chips from typed backend codes. No probability, confidence percentage, or inferred lineage shown.
...
- Don't show probability, confidence percentage, generated explanations, or inferred lineage in Related Assets. Use honest labels: "Same Recipe", "Visual Variants". No semantic search, AI similarity, or lineage detection language.
