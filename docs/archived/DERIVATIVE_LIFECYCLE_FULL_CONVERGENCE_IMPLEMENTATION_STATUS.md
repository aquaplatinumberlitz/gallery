...
- Added `test_phase_7_scan_converges_thumbnail_and_preview_files` — the
  scan-to-ready file convergence acceptance test. It exercises the full pipeline:
  library registration, manual scan, automatic queuing of both configured
  variants (thumbnail + preview), and scheduler worker materialization of both
  files to `ready` status in the thumbnail cache. The test passes as part of the
  Phase 7 implementation and verifies end-to-end convergence.
- Executed full repository verification with `./test.sh fast`: backend API,
  frontend unit, lint, docs, and production build. All gates pass.
- Documented `evicted` and `deferred_capacity` derivative states, absolute
  shutdown deadline, and phase-gated lifecycle ownership in maintained docs.
- Resolved final audit workstreams: capacity-deferred oscillation backoff
  cooldown, active-stop reference counting for concurrent `stop()` linearization,
  per-library bytes alongside global quota usage in the status API, integrity
  repair counts based on actual reconciliation outcomes, and admin UI per-kind
  queued/running/failed/deferred lifecycle rows.
- Archived completed derivative lifecycle plans to `docs/archived/`.
- Updated `docs/plans/README.md` and `docs/archived/README.md` for the archival.
...
deliverables are verified passing; and the completed derivative lifecycle plans
are archived with maintained documentation updated.
...
closeout (full repository verification, `./test.sh fast`, archival) is complete;
see Phase 7 above.
