...
| `GALLERY_DERIVATIVE_SHUTDOWN_TIMEOUT_SECONDS`          | integer, minimum 1      | `30`                                                  | Per-worker bounded join timeout used by `stop()`. A timeout leaves in-flight renders running and is recorded as an incomplete (not clean) shutdown. |
