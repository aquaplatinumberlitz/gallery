...
| Phase 3 — Frontend Vitest coverage >90% | In progress | See supplement plan (frontend-vitest-coverage-supplement.md) for F0-F8 phases. F0-F4 built the foundation: API services 94.4%, stores 92.9%, utils 100%, shared infra created, baseline ~24% lines. F5-F7 landed: Vue Query composables 91.3% lines, large Vue components 34.4% lines (below 60% target), total 53% lines. F8 remains to ratchet thresholds to final 90/90/85/80 target. |
