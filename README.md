...
| `GET` / `POST`         | `/api/search/index-jobs/{id}[ /cancel]`         | Inspect or idempotently cancel a durable search-index job                                         |
...
| `GET`                  | `/api/facets`                                   | Typed folder/library/all faceted counts (tool, model, sampler, etc.)                             |
