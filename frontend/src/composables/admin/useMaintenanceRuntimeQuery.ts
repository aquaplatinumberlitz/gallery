...
export function runtimeHasActiveWork(data: MaintenanceRuntimeResponse | undefined): boolean {
  ...
  if (gr.catalog_active_jobs > 0) return true;
  if (gr.metadata_active_jobs > 0) return true;
  if (gr.derivative_active_jobs > 0) return true;
  const lc = data.metadata_lifecycle;
  if (lc) {
    if (lc.running_metadata_jobs > 0) return true;
  }
  return false;
}
