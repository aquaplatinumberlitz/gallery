...
export const CORE_PARAMS = new Set(["Seed", "Steps", "CFG", "Sampler", "Scheduler", "AspectRatio"]);
...
export function hasCoreParams(params: Record<string, unknown> | null | undefined): boolean {
...
export function hasSecondaryParams(params: Record<string, unknown> | null | undefined): boolean {
  if (!params) return false;
...
export function hasModelData(meta: MetadataResponse | null | undefined): boolean {
...
export function getExtraParamKeys(params: Record<string, unknown> | null | undefined): string[] {
  if (!params) return [];
  const known = new Set([...CORE_PARAMS, ...SECONDARY_PARAM_KEYS, "Model", "Lora", "Width", "Height", "SwarmVersion"]);
  return Object.keys(params).filter((k) => !known.has(k));
}

export function hasAdvancedData(meta: MetadataResponse | null | undefined): boolean {
  if (!meta) return false;
