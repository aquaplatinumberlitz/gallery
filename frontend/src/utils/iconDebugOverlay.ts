...
function collectSvgMetrics(el: SVGElement, index: number): SvgMetrics {
  ...
  return {
    ...
    buttonOpacity:
      button instanceof HTMLElement ? parseFloat(getComputedStyle(button).opacity) : null,
...
function collectData(): DebugData {
  return {
    ...
    tabletHeaderIcons: Array.from(
      document.querySelectorAll<SVGElement>('.tablet-header svg')
    ).map(collectSvgMetrics),
    tabletToolbarIcons: Array.from(
      document.querySelectorAll<SVGElement>('.tablet-gallery-toolbar svg')
    ).map(collectSvgMetrics),
  };
}
