...
function filtersToValues(filters: FieldFilter[]): FormValues {
  ...
  for (const f of filters) {
    switch (f.field) {
      case "prompt":
      case "positive":
        values.prompt = f.value;
        break;
      case "negative":
        values.negative = f.value;
        break;
      case "model":
        values.model = f.value;
        break;
      case "sampler":
        values.sampler = f.value;
        break;
      case "scheduler":
        values.scheduler = f.value;
        break;
      case "lora":
        values.lora = f.value;
        break;
      ...
      case "folder":
      case "path":
        values.folder = f.value;
        break;
      case "name":
        values.name = f.value;
        break;
      case "seed":
        values.seed = { value: f.value, op: f.operator || "=" };
        break;
      case "steps":
        values.steps = { value: f.value, op: f.operator || "=" };
        break;
      case "cfg":
        values.cfg = { value: f.value, op: f.operator || "=" };
        break;
      case "width":
        values.width = { value: f.value, op: f.operator || "=" };
        break;
      case "height":
        values.height = { value: f.value, op: f.operator || "=" };
        break;
      case "clip_skip":
        values.clip_skip = { value: f.value, op: f.operator || "=" };
        break;
      case "denoising_strength":
      case "denoising":
        values.denoising_strength = { value: f.value, op: f.operator || "=" };
        break;
      case "hires_upscale":
        values.hires_upscale = { value: f.value, op: f.operator || "=" };
        break;
      case "hires_steps":
        values.hires_steps = { value: f.value, op: f.operator || "=" };
        break;
      case "ratio":
        values.ratio = f.value;
        break;
      case "size":
        values.size = f.value;
        break;
      ...
      default:
        values.raw = values.raw
          ? `${values.raw} ${f.field}:${f.operator || ""}${f.value}`
...
function collectFilters(values: FormValues): FieldFilter[] {
  ...
  if (values.clip_skip.value)
    add("clip_skip", values.clip_skip.value, values.clip_skip.op !== "=" ? values.clip_skip.op : undefined);
  if (values.denoising_strength.value)
    add(
      "denoising_strength",
      values.denoising_strength.value,
      values.denoising_strength.op !== "=" ? values.denoising_strength.op : undefined,
    );
  if (values.hires_upscale.value)
    add(
      "hires_upscale",
      values.hires_upscale.value,
      values.hires_upscale.op !== "=" ? values.hires_upscale.op : undefined,
    );
  if (values.hires_steps.value)
    add("hires_steps", values.hires_steps.value, values.hires_steps.op !== "=" ? values.hires_steps.op : undefined);
...
<template>
  <Sheet :open="isOpen" :modal="false" @update:open="handleOpenChange">
    <SheetContent
      side="right"
      class="advanced-search-drawer w-[420px] max-w-[90vw] p-0 sm:max-w-[420px]"
      data-testid="advanced-search-drawer"
    >
      <div class="advanced-search-drawer-inner">
        <SheetHeader class="advanced-search-header">
          <SheetTitle class="text-base font-semibold">Advanced Search</SheetTitle>
          <SheetDescription class="sr-only">
            Build detailed metadata and file filters for the gallery.
          </SheetDescription>
        </SheetHeader>
                ...
                <form.Field name="seed" v-slot="{ field: f }">
                  ...
                  <div class="numeric-row">
                    <select
                      ...
                      class="numeric-op-select"
                      :value="f.state.value.op"
                      aria-label="Seed operator"
                      @change="
                    ...
                    <Input
                      ...
                      :model-value="f.state.value.value"
                      @update:model-value="(v: string) => f.handleChange({ value: v, op: f.state.value.op })"
                      placeholder="12345"
                      type="text"
                      variant="default"
                ...
                <form.Field name="denoising_strength" v-slot="{ field: f }">
                  ...
                  <div class="numeric-row">
                    <select
                      ...
                      class="numeric-op-select"
                      :value="f.state.value.op"
                      aria-label="Denoising Strength operator"
                      @change="
                        f.handleChange({ value: f.state.value.value, op: ($event.target as HTMLSelectElement).value })
                      "
                    >
                ...
                <form.Field name="hires_upscale" v-slot="{ field: f }">
                  ...
                  <div class="numeric-row">
                    <select
                      ...
                      class="numeric-op-select"
                      :value="f.state.value.op"
                      aria-label="HiRes Upscale operator"
                      @change="
                        f.handleChange({ value: f.state.value.value, op: ($event.target as HTMLSelectElement).value })
                      "
                    >
                ...
                <form.Field name="hires_steps" v-slot="{ field: f }">
                  ...
                  <div class="numeric-row">
                    <select
                      ...
                      class="numeric-op-select"
                      :value="f.state.value.op"
                      aria-label="HiRes Steps operator"
                      @change="
                        f.handleChange({ value: f.state.value.value, op: ($event.target as HTMLSelectElement).value })
                      "
                    >
              ...
              <div class="field-grid">
                ...
                <form.Field name="advanced" v-slot="{ field }">
                  ...
                  <Input
                    ...
                    :model-value="field.state.value"
                    @update:model-value="(v: string) => field.handleChange(v)"
                    placeholder="Advanced field value"
                    type="text"
                    variant="default"
                    class="field-input"
                  />
                ...
                <form.Field name="raw" v-slot="{ field }">
                  ...
                  <Input
                    ...
                    :model-value="field.state.value"
                    @update:model-value="(v: string) => field.handleChange(v)"
                    placeholder="e.g. model:PonyXL sampler:Euler a"
                    type="text"
                    variant="default"
                    class="field-input"
                  />
