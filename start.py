...
def check_pip_health(pip_exec):
    ...
    try:
        ...
        subprocess.run([str(pip_exec), "--version"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True)
        return True
...
def setup_frontend():
    ...
    # Keep frontend installs pinned to the package manager declared in package.json.
    pkg_manager = "corepack pnpm" if shutil.which("corepack") else "pnpm"
